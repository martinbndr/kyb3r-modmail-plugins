...
class SupportTimes(commands.Cog):
    ...
    async def load_schedules_startup(self):
        ...
        if self.schedules_loaded is False:
            schedule_timezone = (
                None if self.config["timezone"] is None else pytz.timezone(self.config["timezone"])
            )
            if not len(self.config["enable_schedules"]) == 0:
                for i in self.config["enable_schedules"]:
                    enable_scheduler = aiocron.crontab(
                        i, func=self.enable_modmail, tz=schedule_timezone, start=True
                    )
            ...
            if not len(self.config["disable_schedules"]) == 0:
                for i in self.config["disable_schedules"]:
                    disable_scheduler = aiocron.crontab(
                        i, func=self.disable_modmail, tz=schedule_timezone, start=True
                    )
    ...
    async def support_times_show(self, ctx: commands.Context):
        ...
        configured_timezone = (
            "None (uses default system time)" if self.config["timezone"] is None else self.config["timezone"]
        )
        ...
        embed2 = discord.Embed(
            description=f"Modmail enable schedules:\n{schedules[0]}\n\nModmail disable schedules:\n{schedules[1]}",
            color=self.bot.main_color,
        )
    ...
    async def support_times_scheduleadd(self, ctx: commands.Context, mode: str = None, *, cron: str = None):
        ...
        if not mode_str in ["disable", "enable"]:
            embed = discord.Embed(
                description=f"The Mode needs to be ``disable`` or ``enable``.", color=self.bot.error_color
            )
            return await ctx.send(embed=embed)
        valid_cron = croniter.croniter.is_valid(cron)
        if not valid_cron:
            embed = discord.Embed(
                description=f"The cron job you provided, is invalid.", color=self.bot.error_color
            )
            view = discord.ui.View().add_item(
                discord.ui.Button(
                    style=discord.ButtonStyle.link,
                    label="Cronjob Examples",
                    url="https://crontab.guru/examples.html",
                )
            )
            return await ctx.send(embed=embed, view=view)
        if cron in enable_schedules or cron in disable_schedules:
            embed = discord.Embed(
                description=f"You already have this cronjob added to either enabling or disabling schedules.",
        ...
        embed = discord.Embed(
            description=f"Successfully added schedule ``{cron}``!\nIf this schedule will run it is going to **{mode_str}** modmail.",
            color=discord.Color.green(),
        )
        embed.add_field(
            name="Good to know",
            value=f"If the added schedule won´t **{mode_str}** modmail or it will run at a wrong time, set your timezone.",
        )
        return await ctx.send(embed=embed)
    ...
    async def support_times_scheduleremove(self, ctx: commands.Context, *, cron: str = None):
        ...
        embed = discord.Embed(
            description=f"Successfully removed schedule ``{cron}``!", color=discord.Color.green()
        )
        return await ctx.send(embed=embed)
    ...
    async def support_times_mode(self, ctx: commands.Context, mode: str = None):
        ...
        if not mode_input in ["new", "all"]:
            embed = discord.Embed(
                description="Invalid mode. It needs to be either ``new`` or ``all``.",
                color=self.bot.error_color,
            )
            return await ctx.send(embed=embed)
        ...
        embed = discord.Embed(
            description=f'Automatic disabling schedules will disable **{("new threads" if new_mode == 1 else "all threads")}**.',
            color=discord.Color.green(),
        )
        return await ctx.send(embed=embed)
    ...
    async def support_times_timezone(self, ctx: commands.Context, timezone: str = None):
        ...
        if timezone_input == "none":
            ...
            embed = discord.Embed(
                description="The timezone has been set to default. It now uses the default system time.",
                color=discord.Color.green(),
            )
            return await ctx.send(embed=embed)
        ...
        try:
            ...
            embed = discord.Embed(
                description="The timezone you provided is invalid.", color=self.bot.error_color
            )
            return await ctx.send(embed=embed)
        ...
        embed = discord.Embed(
            description=f'The timezone has been set to ``{self.config["timezone"]}``.',
            color=discord.Color.green(),
        )
        return await ctx.send(embed=embed)
    ...
    async def support_times_log(self, ctx: commands.Context, mode: bool = None):
        ...
        embed = discord.Embed(
            description=f'The logging has been **{("enabled" if mode else "disabled")}**.',
            color=discord.Color.green(),
        )
    ...
    async def enable_modmail(self):
        ...
        if self.bot.config["dm_disabled"] != DMDisabled.NONE:
            ...
            if self.bot.config["log_channel_id"] is not None:
                ...
                if log_channel:
                    embed = discord.Embed(
                        title="Modmail is now enabled!",
                        description=f"Modmail has been automatically enabled.",
